...
object LeaderAutoDownRolesSpec {
  ...
  private val memberRoles = Set("testRole", "dc-1")
  private val testRoles = Set("testRole")
  private val memberA = TestMember(Address("akka.tcp", "sys", "a", 2552), Up, memberRoles)
  private val memberB = TestMember(Address("akka.tcp", "sys", "b", 2552), Up, memberRoles)
  private val memberC = TestMember(Address("akka.tcp", "sys", "c", 2552), Up, memberRoles)
  private val memberD = TestMember(Address("akka.tcp", "sys", "d", 2552), Up, Set("otherRole", "dc-1"))

  object LeaderAutoDownRolesTestActor {
    def apply(targetRoles: Set[String], autoDownUnreachableAfter: FiniteDuration, probe: ActorRef): LeaderAutoDownRolesTestActor =
      new LeaderAutoDownRolesTestActor(targetRoles, autoDownUnreachableAfter, probe)
  }
...
class LeaderAutoDownRolesSpec extends AkkaSpec(ActorSystem("LeaderAutoDownRolesSpec")) {
  ...
  def autoDownActor(autoDownUnreachableAfter: FiniteDuration): ActorRef =
    system.actorOf(Props(LeaderAutoDownRolesTestActor(testRoles, autoDownUnreachableAfter, testActor)))
